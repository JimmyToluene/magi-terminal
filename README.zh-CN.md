...
每一个色值都能追溯到画面上真实出现过的东西，或是四种官方配色之一；每一个色槽都经过对黑底的对比度实测。字体则是 NERV 显示屏所参照的、真实的 NEC PC-9801 ROM 字形，并且完全基于自由的洁净室素材构建，不涉及任何受版权保护的固件。

配色通过转义序列由你 SSH **登入的那台机器**下发，因此外观跟着工作站走，无论你从哪个客户端连进来都一样。

## 两套配色

### MAGI 单色（默认）

即 MAGI 那些琥珀底黑的文字画面。全部 16 个 ANSI 色槽都是同一条渐变带上的取样点，这条渐变带在两种官方配色之间插值：NERV 橙 `#F66E25` 与 EVA 黄 `#F6E201`。每一个色槽都落在这条带上。
...
**如果你在用 Powerlevel10k，请把这段放在 p10k 的 instant prompt 代码块之前，也就是整个文件的最前面。** instant prompt 会捕获 `.zshrc` 写往控制台的所有输出，因此在它之后发出的转义序列永远到不了终端。这个失败是无声的，而且很容易被误判：登录时配色毫无反应，可事后手动 `source magi.sh` 却完全正常，看上去就像是客户端的问题。把调用丢到后台也没有用，因为子 shell 继承的是同一个被捕获的 stdout。
...
NERV 的屏幕并没有官方色值规范。那是赛璐璐与早期数字动画，制作时本就没有一套 VI 系统。但有三样东西是**可考的**，本项目的一切都建立在它们之上：
...
最后这一点决定了整个设计。在《EVA》里，屏幕上的颜色承载着**信息**。所以在 NERV 控制台配色中，每个色槽的色相都有其来由：

| 色槽 | 色值 | 画面中的来源 |
|---|---|---|
| 背景 | `#000000` | 标题卡的纯黑，始终是纯黑 |
...
背景固定在 `#000000`，不做任何向灰的柔化。标题卡与 MAGI 画面都是纯黑，一旦妥协，整个观感就垮了。

## 重建这些画面

下面每一张画面都是从零绘制的。`tools/make-nerv-screens.py` 只用上面这套配色，加上本仓库自己构建的 PC-9800 字体，画布尺寸 640×400（那正是这些显示器所依据的 PC-9801 的原生分辨率），因此它们既是完全原创的素材，同时也是这套配色的实机演示。

```bash
python3 tools/make-nerv-screens.py -o images/
```

**MAGI 决议画面。** 三套系统、三次独立判断，而那唯一的一票反对，仅靠警报红就标示了出来。投出否决的是 CASPER：承载赤木直子「作为女人」那一面的单元，这也正是 MAGI 陷入僵局能够推动剧情的原因。

![MAGI 决议画面](images/nerv-magi.png)

**パターン青（蓝色波形）。** 全片对颜色最吃重的一次使用：蓝色意味着目标的波形与使徒吻合。这一屏就是 NERV 控制台配色存在的全部理由，因为只有当蓝色仍是蓝色时，这份读数才承载信息。
...
亮度按**终端中的用途**分配。一个色槽在工作屏幕上被读得越多，它就越亮。
...
第七话里 `Jet Alone` 的启动画面以 **NEC PC-9800 系列**为原型，那是当时日本占据主导地位的 PC 平台。开头几行与真实 PC-9801 的 BIOS 内存检测一致，而 `addr PSP blks size` 那张表则是 `VMAP.COM` 的输出，一个相当冷门的日本内存诊断工具。画面上的字形，是烧录在 PC-9801 ROM 中的 8×16 ANK 点阵字体。它带有粗衬线，这正是这块屏幕永远不会被看成「西方 DOS 窗口」的原因；IBM 的 VGA CP437 字体基本上是无衬线的。
...
`tools/freecg98-to-fontrom.py` 彻底绕开了对 NEC 固件的需求。它读取 `FREECG98.BMP`，也就是随 DOSBox-X 分发的自由洁净室 Anex86 兼容 PC-98 字体，并按照 ttf-pc9800 解析器所期望的字节布局写出一份 `FONT.ROM`：
...
- `FREECG98.BMP` 是 NEC ROM 的**复刻**。它保留了 PC-98 的气质与正确的 8×16 度量，字形则接近原作而未做到完全一致。如果你需要精确复原，把真实硬件的 dump 放进去重新 `make` 即可。

该字体在其 16px 字身的整数倍处最为锐利。如果看起来发虚，请先在 16 与 32 之间切换，再考虑逐级微调。
...
**字体永远在客户端侧。** 服务端发送的是字节，渲染字形的是客户端。没有任何转义序列能改变这一点，所以每一台设备都要各自安装该 TTF。
